...
func (p *parser) parseAttribute() (*Decl, error) {
	...
	if quoted {
		...
		if _, err := p.mustHaveNext(quoteToken); err != nil {
			fmt.Printf("parseAttribute: Missing closing quote")
			return nil, err
		}
	}
...
func (p *parser) parseCondition() (*Decl, error) {
	...
	case InToken:
		...
		return attributeDecl, nil
	case IsToken:
		fmt.Printf("parseCondition: IsToken\n")
		...
		if p.cur().Token == NotToken {
			fmt.Printf("parseCondition: NotToken\n")
		...
		if p.cur().Token == NullToken {
			fmt.Printf("parseCondition: NullToken\n")
